...
#define EOS '\0'
	...
	{
		return (charEqualsEOS(str[count]) == -1 ?
				strlen_ceImpl(str, count + 1) : count);
	}
	...
	{
		return (charEquals(a[startIndex], b[startIndex]) >= 0 ?
				(charEqualsEOS(a[startIndex]) == -1 ?
						strcmp_ceImpl(a, b, startIndex + 1) : 0) :
				-1);
	}
